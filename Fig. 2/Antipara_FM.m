...
eigenvector = zeros(N*4+1,N*4,length(kx),length(ky));


% Initialize storage for frequencies
a_magnon = NaN(4*N, length(kx));
c_magnon = NaN(4*N, length(kx));
a_antimagnon = NaN(4*N, length(kx));
c_antimagnon = NaN(4*N, length(kx));

% Define a tolerance for imaginary parts
tol = 1e6;
...
for m=1:length(ky)
    for n = 1:length(kx)
        ...
        D1P = 1/2*gamma*u0.*f1.* ((k+(kx(n))^2/k)+2*kx(n));
        D2P = 1/2*gamma*u0.*f2.* ((k+(kx(n))^2/k)+2*kx(n)); 


        D3P = 1/2*gamma*u0.*f2.* ((k+(kx(n))^2/k)-2*kx(n));
        D4P = 1/2*gamma*u0.*f1.* ((k+(kx(n))^2/k)-2*kx(n));

        D1N = 1/2*gamma*u0.*f1.* (-(k+(kx(n))^2/k)+2*kx(n));
        D2N = 1/2*gamma*u0.*f2.* (-(k+(kx(n))^2/k)+2*kx(n));

        D3N = 1/2*gamma*u0.*f2.* (-(k+(kx(n))^2/k)-2*kx(n));
        D4N = 1/2*gamma*u0.*f1.* (-(k+(kx(n))^2/k)-2*kx(n));

        delta1P_ = 0;%interlayer
        delta2P_ = 0;
        delta1N_ = -delta1P_;
        delta2N_ = -delta2P_;

        DELTA1P = gamma*u0*Ms1/2*((1-N1)*(kx(n)/k)^2-N1); %intralayer
        DELTA2P = gamma*u0*Ms2/2*((1-N2)*(kx(n)/k)^2-N2);%这一项会导致antic和magnonacrossing 巨大??
        DELTA1N = -DELTA1P;
        DELTA2N = -DELTA2P;
        ...
        for i =1:4*N
            ...
            angluar_f_obc(n,m,:) = egv/2/pi/1e9;
            eigenvector (1,:,n,m) = egv/2/pi/1e9;
            eigenvector(2:4*N+1,:,n,m) = v;
%             
        end 
        H_all_obc(:,:,n) = H_obc;
...
figure;
%BIG
ax = axes('Units', 'inches', 'Position', [2 2 4.5 3.5]); %left=1, bottom=1, width=4.5 inches, height=3.5 inches
%small
...
hold on;
% Plot a-magnon
scatter(kx(idx), squeeze(All_grouped_eigenvalues(1,:,idx)), 15, 'MarkerEdgeColor', dark_blue, ...
...
% Plot c-magnon
scatter(kx(idx), squeeze(All_grouped_eigenvalues(2,:,idx)), 15, 'MarkerEdgeColor', orange, ...
    'MarkerFaceColor', orange, 'Marker', magnon_marker, ...
    'MarkerFaceAlpha', magnon_alpha, 'MarkerEdgeAlpha', magnon_alpha, ...
    'DisplayName', 'c-magnon');

% Plot a-antimagnon with 90% transparency
scatter(kx(idx), squeeze(All_grouped_eigenvalues(3,:,idx)), 15, 'MarkerEdgeColor', dark_blue_anti, ...
...
scatter(kx(idx), squeeze(All_grouped_eigenvalues(4,:,idx)), 15, 'MarkerEdgeColor', orange_anti, ...
    'MarkerFaceColor', antimagnon_face, 'Marker', antimagnon_marker, ...
    'MarkerFaceAlpha', antimagnon_alpha, 'MarkerEdgeAlpha', antimagnon_alpha, ...
...
legend(legend_handles, legend_labels, ...
    ...
    'FontSize', 26, ...                    % Set desired font size
...
set(gca, 'LineWidth', 2.5,...
         'FontName', 'Arial', ...
         'FontSize', 26, ...                    % Increased font size for ticks
...
%% code for frequency domain COMSOL to overcome table2array ---cells
clc; clear all; %close all

%coordinate setting
ksCOM2 = 1/(3e-9); %sampling frequency for x
xCOM2 = linspace(-600*2+3,600*2-3,801-2); %
fCOM2 = 0:0.1:60;
...
z1COM1 = abs(flipud(transpose(DP1COM1)));%Top Py
z1COM2 = abs(flipud(transpose(DP1COM2)));%Top YIG
z1COM3 = abs(flipud(transpose(DP1COM3)));%Bottom Py
z1COM4 = abs(flipud(transpose(DP1COM4)));%Bottom YIG
...
fCOM2 = 60:-0.1:0;
im = imagesc(kxCOM2,fCOM2,(z1COM2+z1COM1+z1COM4+z1COM3)/3);
